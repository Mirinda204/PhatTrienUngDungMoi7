...
async function createUserWithRandomPassword(userData, userController, session = null) {
    try {
        ...
        try {
            ...
            console.log(`✅ Password email sent to ${newUser.email}`);
        } catch (emailError) {
            console.error(`⚠️  Failed to send password email to ${newUser.email}:`, emailError.message);
            // Vẫn trả về thành công vì user đã được tạo
            // Email có thể thử gửi lại sau
        }
